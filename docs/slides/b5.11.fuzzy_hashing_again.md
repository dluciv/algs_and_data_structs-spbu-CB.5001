...
$h_i \in H$ иногда определяются через одну $h \in H$, как $h_i = (h \circ \mathit{perm}_i): A \rightarrow I$
...
Пусть у нас есть 64-битный SimHash $10^7 < 2^{24}$ по документам. Можно построить несколько отсортированных индексов по подмножествам этих битов и по докумменту искать, похожие в этих индексах.
...
* Опишите принцип Locality Sensetive Hashing
* Опишите пример применения LSH для поиска

## Упражнения

* На любом языке програмирования реализуйте систему индексирования и поискаа документов, использующую LSH и расстояние Хэммиинга